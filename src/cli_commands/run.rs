...
pub async fn cmd_run(agent_cmd: String, args: Vec<String>) -> Result<(), String> {
    ...
    // Flattened header with explicit \r\n terminator for automatic entry
    ...
    let stdin_handle = tokio::spawn(async move {
        ...
        while let Ok(n) = stdin.read(&mut buffer).await {
            ...
            if master_writer.write_all(chunk).is_err() { break; }
            let _ = master_writer.flush();
        }
    });

    // Instant Injection Task (500ms delay to land after agent banner)
    let mut master_writer_for_inj = pair.master.take_writer().map_err(|e| e.to_string())?;
    tokio::spawn(async move {
        tokio::time::sleep(std::time::Duration::from_millis(500)).await;
        let _ = master_writer_for_inj.write_all(injection_header.as_bytes());
        let _ = master_writer_for_inj.flush();
    });
